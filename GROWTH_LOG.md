...
## 2026-07-21 — Article #8: new category, home gym & fitness (Jack's direct request)

Jack asked directly for a new category expansion: home gym/fitness, anchored by two specific viral TikTok items he named — an "adjustable chest workout trainer" and a pushup board ("find a cheap one"). Not a beauty-lane item, so no territory conflict with viralfinds; built on trendtrackr per the existing split. TrendTrackr's own `CONTENT_BACKLOG.md` already had a researched "Home gym & fitness" brief from 2026-07-18 (small-space/apartment angle), so this article follows that brief's structure and fills the remaining 3 slots from its candidate list.

**IMPORTANT verification caveat — different from every prior article:** this session ran in the cloud with no browser/Chrome access (the scheduled/coordination environment this run inherited doesn't have the desktop bridge other sessions have used to hand-verify Amazon listings). Amazon's own product and Best-Sellers pages are also robots-disallowed for the automated fetch tool available here. So instead of hand-browsing Amazon, product data below is sourced from a third-party Amazon sales-rank/ratings aggregator (asinsight.com), cross-checked across independent pulls where possible, plus Amazon search-result links confirming each ASIN is a real, live listing. Numbers are presented rounded (~) in the article itself to flag the lower-confidence sourcing, and the article's own "How we picked" section carries the same disclosure for readers/Jack. **Recommend a quick live-Amazon spot-check of all 5 ASINs next time someone's in Jack's Chrome before this one gets pinned hard or promoted** — same standard CONTENT_BACKLOG.md already holds unshipped briefs to.

**Five picks:**
- Twister Arm Trainer — Adjustable Chest Workout Equipment, 30-120lb (ASIN B0DRV8MDR5, ~4.4★/~1,700 ratings, ~$25-27) — the exact TikTok "chest twister" trend Jack named. Note: this is a crowded, lightly-branded Amazon niche with many near-identical competing listings; flagged as a con in the article itself.
- Push-Up Board w/ Ergonomic Bracket Stands (ASIN B07SFDS8RW, ~4.4★/~7,100 ratings, ~$9.99) — the cheap pushup board Jack asked for; this was the single most cross-corroborated data point in the research (identical numbers across two independent aggregator pulls).
- Fit Simplify Resistance Loop Bands, 5-pack (ASIN B01AVDVHTI, ~4.5★/~135,000 ratings, ~$9.98) — #1 in its category, also the exact brand CONTENT_BACKLOG.md's brief had already flagged as a candidate.
- Ally Peaks Doorway Pull-Up Bar (ASIN B08MY13HYC, ~4.4★/~12,300 ratings, ~$27.99) — swapped in over the backlog's guessed brands (Iron Gym/ProsourceFit) since this is what the actual sales data showed leading the category.
- Amazon Basics Extra-Thick Exercise Mat w/ carrying strap (ASIN B01LP0U5X0, ~4.6★/~69,600 ratings, ~$22.48) — floor protection/quiet-cardio slot from the backlog brief.

**Shipped this run:** `articles/best-tiktok-viral-home-gym-fitness.html` (full template: quick picks, compare table, 5 product blocks with honest cons, ttTrack labels chesttrainer_/pushupboard_/bands_/pullup_/mat_); `index.html` homepage card + nav link; `sitemap.xml` entry; `images/og-home-gym-fitness.png` + `images/pin-home-gym-fitness.png` (original on-brand graphics generated locally, no product photography, consistent with policy). PIN_QUEUE.md got a QUEUED article-level entry, explicitly annotated to hold for the spot-check above before posting — nothing was posted anywhere this run (posting stays human-clicked per policy regardless).

**Also noted, not fixed this run:** article #7 (pet tech, 2026-07-20) has no corresponding GROWTH_LOG entry even though it's live on the site/homepage/sitemap — looks like a logging gap from that run, not something wrong with the pet tech page itself. Flagging so it doesn't look like this run's oversight.

**Next priority:** live-Amazon spot-check of the 5 fitness ASINs above (needs Jack's Chrome); GSC indexing request for the new URL (also needs browser); per-product pp-*.png images + individual PIN_QUEUE entries for the 5 fitness products if Jack wants this category pinned at the same granularity as water bottles/home org.


## 2026-07-23 — Live-Amazon verification of the home gym article (all 5 ASINs hand-checked; push-up pick corrected)

**Context:** Article #8 (home gym & fitness, 2026-07-21) shipped with a stated caveat — that run had no Chrome/Amazon access, so its 5 products were sourced from a third-party aggregator (asinsight.com), and it explicitly asked for a live-Amazon spot-check of all 5 ASINs "next time someone's in Jack's Chrome before this one gets pinned hard or promoted." This run had Chrome, so that spot-check was the highest-leverage move available — an entire live article was resting on lower-confidence data.

**All 5 ASINs hand-verified against their live Amazon.com product pages today (2026-07-23).** Four checked out exactly: B0DRV8MDR5 NOONCRAZY Twister Arm Trainer (4.4 stars, 1,787 ratings, $25.99, Amazon's Choice, 3K+ bought/mo); B01AVDVHTI Fit Simplify Resistance Loop Bands 5-pack (4.5 stars, 136,208 ratings, $9.98, #1 Best Seller / Amazon's Choice, 30K+/mo — the "#1 resistance band set" claim holds); B08MY13HYC Ally Peaks Doorway Pull-Up Bar (4.4 stars, 13,416 ratings, $27.99, Amazon's Choice, 10K+/mo, 440lb); B01LP0U5X0 Amazon Basics Extra-Thick Exercise Mat (4.6 stars, 69,739 ratings, $22.48, Amazon's Choice, 20K+/mo). All four matched the article's rounded numbers.

**The push-up pick needed correcting (the honesty item).** ASIN B07SFDS8RW is real, in stock, 4.5 stars, 7,361 ratings, $9.99, Amazon's Choice, 5K+ bought/mo — so its price/rating band matched — BUT the live listing is a pair of ergonomic push-up BARS/handles (Redipo), not the "color-coded push-up board that maps out hand positions" the article described. The aggregator got the numbers right but the product identity wrong. Kept the ASIN (it's a genuinely strong sub-$10 Amazon's Choice pick) and rewrote its name, verdict, body, pros/cons and quick-pick card to accurately describe ergonomic push-up bars (deeper range of motion, wrist-neutral foam grips, removable bases), and renamed its ttTrack labels pushupboard_* to pushupbars_*.

**Also upgraded in the same edit:** every product's rounded (~) figure replaced with the exact hand-verified count; Amazon's Choice / #1 Best Seller badges and "N bought in past month" signals added to each rating line and the compare table; the page meta line changed to "Verified against live Amazon.com listings"; and the "How we picked" note rewritten from "aggregator-sourced, read before promoting hard" to an honest live-verified-2026-07-23 statement that also discloses the push-up correction. Net effect: article #8 moves from "lower-confidence, don't pin hard" to fully live-verified — it can now be pinned/promoted like the others.

**Deployed:** replaced articles/best-tiktok-viral-home-gym-fitness.html via GitHub's upload UI (commit a42f89b, "Home gym article: live-Amazon verification (2026-07-23), fix push-up pick description"; 91 commits total). Confirmed live after the Pages rebuild via a cache-busted URL — the page renders "Push-Up Bars with Ergonomic Stands" and "7,361 ratings, Amazon's Choice." index.html and sitemap.xml already carried this article (added 2026-07-21), so nothing was needed there.

**Checks skipped this run (noted honestly):** did not open Google Search Console or Amazon Associates — the focus was the verification/correction + deploy, and neither dashboard was already open in this browser session, so per policy I did not log in. GSC indexing for the home-gym URL and the Associates click/earnings read remain worth doing whenever someone is at those dashboards.

**Next priority:** (1) The home-gym article is now verified, so its PIN_QUEUE.md entry no longer needs the "hold for spot-check" annotation — it is cleared to pin. (2) The four oldest article pages' navs still do not link the newer guides (Travel / Pet Tech / Home Gym) — a small nav-consistency cleanup whenever those files are next touched. (3) Next new category from CONTENT_BACKLOG (cleaning gadgets / kitchen gadgets flagged) once Chrome is available to hand-verify a fresh ASIN set the same way this run did.


## 2026-07-24 — Article #9: TikTok-viral cleaning gadgets (CleanTok), all 5 live-Amazon verified

**Category pick:** The 2026-07-23 entry flagged "cleaning gadgets" as the next new-category priority once Chrome was available to hand-verify a fresh ASIN set. Chrome was available this run (Jack joined mid-run and approved browser access — the first navigations were auto-denied while no one was present), so cleaning gadgets was the move. It's squarely in trendtrackr's lane — no conflict with viralfinds (beauty). This is a genuinely new category (kitchen gadgets already exists as its own article).

**All 5 picks hand-verified against their live Amazon.com product pages today (2026-07-24)** — star rating, rating count, price and stock read verbatim off each listing: Bissell Little Green Multi-Purpose Portable Carpet Cleaner 1400B (B0016HF5GK) 4.5 stars, 94,003 ratings, $99.99, In Stock, 30K+ bought/mo, "Best Overall" (the most-filmed cleaning machine on CleanTok); Stardrops The Pink Stuff Cleaning Paste 500g (B00DU5SRIY) 4.4 stars, 241,094 ratings, $5.97, In Stock, #5 in All-Purpose Household Cleaners, "Cult Favorite" (most-reviewed product on the entire site, and the cheapest pick); Leebein Electric Spin Scrubber cordless 8-head (B0GF1V16L4) 4.3 stars, 22,839 ratings, $39.98, In Stock, "Best Powered Scrubber" (top organic result for "electric spin scrubber", review leader of a crowded niche); Scrub Daddy Colors Sponges 3-pack (B0CJ1NHWGX) 4.8 stars, 10,979 ratings, $13.99, In Stock, 30K+ bought/mo, "Best Sponge" (highest star rating on the page, the original that started the trend); Drillbrush Drill Brush Attachment Set (B01MRLZ43B) 4.5 stars, 40,876 ratings, $24.98, In Stock, Amazon's Choice / "Overall Pick", "Best Grout Tool". Combined ~409,800 ratings (headlined as "400K+"). Trend angle cross-checked by web search ("CleanTok viral cleaning products 2026") — Pink Stuff, Scrub Daddy, Bissell Little Green, spin scrubbers and drill brushes are all confirmed canonical 2026 CleanTok items, so this is the real intersection of virality + purchasable listing, not rank alone.

**Verification method note:** the Household Cleaning Best Sellers browse node (hpc/3732721) rendered a personalized browsing-history mix this run (its "#1" was AMIR motion-sensor lights, not a cleaning product), not a clean category list — same failure mode the travel run hit with the luggage node. So verification moved to live product detail pages: searched each candidate, opened the top real listing, read ASIN/rating/count/price/stock verbatim via the page DOM. Direct /dp/<ASIN>?tag=jcallahan1542-20 affiliate links used throughout.

**Rejected / passed over (the honesty section):** both the "spin scrubber" and "drill brush" searches are flooded with near-identical clone listings — picked the most-reviewed, badge-holding version of each (Leebein 22.8K; Drillbrush 40.8K + Amazon's Choice) and flagged the clone-crowding as an honest con in the article body. Holikme 6-pc drill brush (81.8K ratings, $8.99) was the higher-review alternative but is a bare brush set, not the full Amazon's-Choice kit — kept Drillbrush. The hpc node's personalized "#1" (AMIR lights) was ignored as off-category.

**Shipped this run (all deployed + verified live):** articles/best-tiktok-viral-cleaning-gadgets.html (full template match: quick-picks jump nav, compare table, 5 product blocks with pros/cons + honest cons, ttTrack labels bissell_/pinkstuff_/spinscrubber_/scrubdaddy_/drillbrush_ across _qp/_table/_button); index.html new homepage card (placed first) + nav link; sitemap.xml new entry (lastmod 2026-07-24) with homepage lastmod bumped; images/og-cleaning-gadgets.png (1200x630) + images/pin-cleaning-gadgets.png (1000x1500), original on-brand graphics, no product photography. Three commits landed (articles/, images/, root); confirmed live after the Pages rebuild via cache-busted URLs — the article renders, the homepage card shows first, and both image assets return HTTP 200.

**GSC:** inspected the new URL — "URL is not on Google / unknown to Google" (expected for a brand-new page) — and Request Indexing succeeded ("URL was added to a priority crawl queue"). Homepage still shows 0 web-search clicks in the overview; property is verified and healthy, just early.

**Amazon Associates (already signed in this session, no login attempted):** Consolidated Earnings Summary, Last 7 Days — 1 click, 0 orders, $0.00 earnings. Unchanged from the 2026-07-20 first-click note. Payment info stays cleared (Jack submitted it 2026-07-20).
...
## 2026-07-28 (later) — Site-wide redesign v2: editorial light theme + real product photos

Jack asked (interactively, mid-session) to make the site "aesthetically pleasing" so "people should want to click," and to "replicate top sites with a lot of traffic." Chose the Wirecutter / The Strategist model: clean white editorial layout, serif (Fraunces) headlines, real product photography, clear award badges, star rating + price up front, one confident coral accent. This directly targets the site's biggest weakness — it was a dark wall of text with zero product imagery.

**Shipped this run (4 commits, all live-verified after the Pages rebuild):**
- **css/style.css** fully reworked into a light, image-first design system. Kept every v1 variable NAME so existing inline styles across all pages still resolve; only flipped the values (dark -> light). Added: product-photo tiles, .card-media (homepage) and .product-media/.product-body two-column product blocks (activated via :has() only when an image is present), .qp-thumb quick-pick thumbnails, compare-table .c-thumb thumbnails, coral CTA buttons, and light-friendly badge colors. Because the CSS is shared, ALL 10 articles instantly moved from the dark text-wall to the clean light editorial theme.
- **index.html**: every homepage card now leads with a real Amazon product photo (the hero pick of each guide) in a clean tile, plus a category eyebrow and serif title.
- **articles/best-tiktok-viral-car-accessories.html** rebuilt as the showcase — product photos in each quick-pick card, in the compare table, and a Wirecutter-style image-left / content-right block for every product.

**Product images:** hotlinked from Amazon's own CDN (m.media-amazon.com, "._AC_SL500_" size token). The main image URL was read off each live product detail page; all 13 (5 car products + 9 homepage heroes) were confirmed to load (HTTP 200) before shipping. As an Associate this is acceptable for a proof-of-concept; can migrate to the Product Advertising API or self-hosted copies later if we want full control.

**Verified live:** homepage cards render with photos (DRIVE organizer, Bissell Little Green, Twister trainer, Veken fountain, OlarHike cubes, AirTag, Fullstar chopper, Vtopmart bins, Owala); the car article renders image-first; and an untouched article (water bottles) confirms the shared CSS degrades gracefully to a clean single-column light layout (badges, gold stars, serif titles, pros/cons cards) even with no product photos yet.

**IMPORTANT for future runs — the template changed.** The site is now image-first / light. Build every NEW article in the new structure: per product use `<div class="product"><a class="product-media">…img…</a><div class="product-body">…</div></div>`, put a `.qp-thumb` image in each quick-pick card, and a `.c-thumb` image in each compare-table row — each pointing at a real Amazon product photo (grab the main image off the live /dp/ page, use the "._AC_SL500_" size). Use **articles/best-tiktok-viral-car-accessories.html as the template, NOT the older text-only water-bottles page.**

**Still open:** the 8 older articles (water bottles, home organization, tech, kitchen, travel, pet tech, home gym, cleaning) now use the new theme and their homepage cards have hero photos, but their in-article product blocks still have no product images. Next high-leverage polish pass: add a real Amazon product photo to each product block in those 8 (same hotlink method) to bring them fully in line with the car-accessories showcase.
